...
def get_parser():
    ...
    parser.add_argument(
        "--data-dir", type=str, help="Input image data"
    )
    parser.add_argument(
...
def train_nn(model, train_loader, test_loader, optimizer, n_epochs, lr):
    ...
    for epoch in range(n_epochs):
        ...
        epoch_f1_score_class_1 = running_class1_correct / (
            running_class1_correct + 0.5 * (running_class0_FN + running_class1_FP))
...
def evaluate_model_on_test_set(model, test_loader):
    ...
    epoch_test_f1 = predicted_correctly_class_1 / (
        predicted_correctly_class_1 + 0.5 * (predicted_class_0_wrong + predicted_class_1_wrong))
...
def save_checkpoint(model, epoch, optimizer, best_f1_score, lr):
    state = {
        'epoch': epoch + 1,
...
def main():
    ...
    imagePatches = glob(os.path.join(args.data_dir, '/**/*.jpg'), recursive=True)
    ...
    optimizer = optim.SGD(resnet18_model.parameters(), lr=args.learning_rate, momentum=0.9,
                          weight_decay=0.003)

    train_nn(resnet18_model, train_loader, test_loader, optimizer, n_epochs=n_epochs, lr=args.learning_rate)
